...
@dp.message_handler(Command("birthday", prefixes="!/"), regexp=r"^[!/]birthday ([a-zA-Z0-9_]+)? (\d\d)?-?(\d\d)-?(\d\d\d\d)?")
async def birthday(message: types.Message):
    ...
    try:
        ...
        await message.answer('I am sorry, *' + message.from_user.first_name + '*,incorrect input', parse_mode='Markdown')
        print (e)
